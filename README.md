...
- All endpoints return consistent error shapes on validation/auth failures: `{ message: string }`.
- Membership checks for projects use the `project_members` table when available; otherwise, the project creator is treated as a member.
- Roles are enforced via middleware; ensure the JWT includes `sub` (user id) and `role`.
